"""ECIS registry/national and Spanish REDECAN estimate adapter."""
...
def parse_ecis_redecan_export(frame: pd.DataFrame) -> pd.DataFrame:
    required = {
        "source_id", "geography_scope", "geography_code", "geography_name", "year",
        "sex", "age_group", "cancer", "measure", "metric", "value", "evidence_type",
    }
    missing = required - set(frame.columns)
    if missing:
        raise ValueError(f"ECIS/REDECAN export is missing columns: {sorted(missing)}")
    rows: list[dict[str, object]] = []
    for source in frame.to_dict("records"):
        source_id = str(source["source_id"]).strip().casefold()
        if source_id not in {"ecis", "redecan"}:
            raise ValueError(f"Unsupported European source: {source_id}")
        ...
        if measure_key not in MEASURES:
            raise ValueError(f"Unsupported European measure: {source['measure']}")
        ...
        if metric_key == "number":
            ...
            raise ValueError(f"Unsupported European metric: {source['metric']}")
        evidence = str(source["evidence_type"]).strip().casefold()
        quality = "national_estimate" if evidence == "modelled" else "population_registry_observed"
        rows.append(
            {
                "source_id": source_id,
                "source_version": "ECIS 2024/2026 release" if source_id == "ecis" else "REDECAN 2025 estimate",
                ...
                "notes": "Registry coverage and national estimates remain separate evidence layers.",
...
def main() -> None:
    ...
    result = parse_ecis_redecan_export(pd.read_csv(args.input))
